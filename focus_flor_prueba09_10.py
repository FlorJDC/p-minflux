...
#            self.updateStats()
    ...
    def single_z_correction(self, feedback_val, initial):
        ...
        if initial:
            ...
            if not self.camON:
                self.camON = True
                self.camera.start.capture() # Thorcam: self.camera.start_live_video(framerate='20 Hz')
                time.sleep(0.200)
...
if __name__ == '__main__':
    ...
    try:
        cam = ids_peak.Camera() # Crea una instancia de la cámara IDS
        cam.open() # Abre la cámara IDS
        # Ajusta las propiedades de la cámara según tus necesidades
        cam.set_pixel_format(ids_peak.PixelFormatName_BGR8)  # Ejemplo de formato de píxel, ajusta según lo necesario
        cam.set_exposure_time(10000)  # Ejemplo de tiempo de exposición en microsegundos, ajusta según lo necesario
        cam.set_frame_rate(30)  # Ejemplo de velocidad de fotogramas, ajusta según lo necesario
        cam.start_capture() 
    except:
