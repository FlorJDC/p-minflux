...
class Frontend(QtGui.QFrame):
    ...
    def craete_roi(self): #Se crea un ROI
...
#        print('Set ROIs function')
        ...
        roinumber = len(self.roilist) #Cantidad de elementos que tiene la lista de coordenadas
        
        if roinumber == 0:
            
            print(datetime.now(), '[xy_tracking] Please select a valid ROI for fiducial NPs tracking')
            
        else: #Sé que hay algo en roilist (porque roinumber !=0), creo el vector coordinates que guarda la info de mi roi
            ...
            for i in range(len(self.roilist)):
                ...
                if DEBUG1:
                    print("valor de roilist en i: ", i, "valores de xmin, ymin: ", xmin,ymin)
                    
                xmax, ymax = self.roilist[i].pos() + self.roilist[i].size()
                if DEBUG1:
                    print("valor de roilist en i: ", i, "valores de xmax, ymax: ", xmax,ymax)
        
                coordinates = np.array([xmin, xmax, ymin, ymax])  
                
                if DEBUG1:
                    print("valor de coordinates en i: ", coordinates)
...
#FC        if DEBUG1:
#FC            print("Estoy en get_image")
        
#-fc        if DEBUG:
#FC            print(datetime.now(),'[xy_tracking-frontend] got image signal')
...
#            self.xyDataMean.setData([xmean], [ymean])
    ...
    def get_backend_states(self, tracking, feedback, savedata):
        if DEBUG1:
            print("Estoy en get_backend_states")

        self.trackingBeadsBox.setChecked(tracking)
        if DEBUG1:
            print("Pasó tracking bead box en get backend states")
        self.feedbackLoopBox.setChecked(feedback)
        if DEBUG1:
            print("Pasó feedbackloop box en get backend states")
        self.saveDataBox.setChecked(savedata)            
        if DEBUG1:
            print("Pasó savedata box en get backend states")
    ...
    def emit_save_data_state(self):
        ...
        if self.saveDataBox.isChecked():
            ...
            if DEBUG1:
                print("SEñal emitida saveDataSignal")
            self.emit_roi_info()
            if DEBUG1:
                print("Llamo a emit_roi_info que emitirá roiInfoSignal")
        else:
            
            self.saveDataSignal.emit(False)
            if DEBUG1:
                print("No està clickeado en save data")
        
    def make_connection(self, backend):
        if DEBUG1:
            print("Estoy en make connection, recibì el backend")
            
        backend.changedImage.connect(self.get_image)
        if DEBUG1:
            print("conecto funcion slot get_image a la señal del backend changedImage")
            
        backend.changedData.connect(self.get_data)
        if DEBUG1:
            print("conecto funcion slot get_data a la señal del backend changedData")
            
        backend.updateGUIcheckboxSignal.connect(self.get_backend_states)
        if DEBUG1:
            print("conecto funcion slot get_backend_states a la señal del backend updateGUIcheckboxSignal")
            
        backend.shuttermodeSignal.connect(self.update_shutter)
        if DEBUG1:
            print("conecto funcion slot update_shutter a la señal del backend shutter_modeSignal que se emite en funciòn shutter handler")
        
        backend.liveviewSignal.connect(self.toggle_liveview)
        if DEBUG1:
            print("conecto funcion slot toogle_liveview a la señal del backend liveviewSignal")

        
    def setup_gui(self):
        if DEBUG1:
            print("estoy en set up GUI")
...
#        self.xyPoint.xyPointPlot.showGrid(x=True, y=True)
        ...
        if DEBUG1:
            print("Se apretó el botón ROI, se creó un ROI con función craete_roi")
        ...
        if DEBUG1:
            print("Se seleccionó un ROI, este click conecta con función emit ROI que emite señan roiInfosignal y va a get_roi_info")
        ...
        if DEBUG1:
            print("Se apretó el botón delete ROI, se elimina ROI con función delete_roi")
        # position tracking checkbox
        
        self.exportDataButton = QtGui.QPushButton('export current data')
        ###########################aqui falta algo?################CREO QUE NO
        ...
        if DEBUG1:
            print("tracking beads box conectado a emit roi info")
        ...
        if DEBUG1:
            print("saveData box conectado a emit_save_data_state que emite saveDataSignal y roiInfoSignal")
        ...
        grid.addWidget(self.xyGraph, 1, 0) #Ajuste de posición de trazas x e y
        grid.addWidget(self.xyPoint, 1, 1) #Ajuste de posiciòn de histograma
        
        self.liveviewButton.clicked.connect(lambda: self.toggle_liveview(self.liveviewButton.isChecked()))
        if DEBUG1:
            print("liveviewButton connected to toggle liveview - line 502")
        
    def closeEvent(self, *args, **kwargs):
        if DEBUG1:
            print("Estoy en close event")
...
class Backend(QtCore.QObject):
    ...
    def toggle_tracking_shutter(self, num, val):
        if DEBUG1:
            print("Estoy en toogle_tracking_shutter. Valor de parámetros recibidos: num: ",num, "val: ", val)
    ...
    def shutter_handler(self, num, on):
        if DEBUG1:
            print("Estoy en shutter_handler. Valor de parámetros recibidos: num: ",num, "on: ", on)
        self.shuttermodeSignal.emit(num, on)
        if DEBUG1:
            print("Se acaba de emitir shuttermodeSignal")
    ...
    def liveview(self, value):
        ...
        '''
        if DEBUG1:
            print("Estoy en liveview, value is:",value)
            
        if value:
            self.camON = True
            print(self.camON)
            self.liveview_start()
            print("arrancó liveview_start")
    ...
    def liveview_start(self): #Esto es como en xyz_tracking - focus
        ...
        if DEBUG1:
            print("Se ejecutó: self.camera.start_live_video()")
    ...
    def liveview_stop(self):
        if DEBUG1:
            print("Estoy en liveview_stop")        
        self.viewtimer.stop()
        print("viewtimer stopped")
        self.camON = False
...
#FC        if DEBUG:
#FC            print(datetime.now(), '[xy_tracking] entered update')
        
        self.update_view()
#FC        if DEBUG1:
#FC            print("luego de llamar a update_view")

        if self.tracking_value:
            if DEBUG1:
                print("tracking_value es verdadero")
                
            self.track()
            if DEBUG1:
                print("se ejecutó llamado a funciòn track()")
                
            self.update_graph_data()
            if DEBUG1:
                print("se ejecutó llamado a función update_graph_data()")
            
            if self.feedback_active:
                if DEBUG1:
                    print("self.feedback_active es verdadero")
                    
                self.correct()
                if DEBUG1:
                    print("se ejecutó llamado a función correct()")
        
        if self.pattern:
            if DEBUG1:
                    print("self.pattern es verdadero")
    ...
    def update_view(self):
        ...
        # acquire image
#FC        if DEBUG1:
#FC            print("Estoy dentro del update_view")
...
#FC        if DEBUG1:
#FC            print("Se emite señal changedImage en update_view, va a parar a get image")
    ...
    def update_graph_data(self):
        ...
        if DEBUG1:
            print("Estoy dentro del update_graph_data")
        if self.ptr < self.npoints:
            ...
            if DEBUG1:
                print("Se emitió señal changed_data, va a para a get_data")
        self.ptr += 1
    ...
    def toggle_tracking(self, val): #esta función es igual a la de xyz_tracking porque es para xy únicamente
        ...
        if DEBUG1:
            print("Estoy en toggle_tracking del backend") 
    ...
    def toggle_feedback(self, val, mode='continous'):
        ...
        '''
        if DEBUG1:
            print("Estoy en toggle_feedback del backend") 
        if val is True:
...
#                                          self.save_data_state)
    ...
    def gaussian_fit(self):
        ...
        if DEBUG1:
            print("Estoy en gaussian_fit del backend") 
...
#            print(datetime.now(), '[xy_tracking] else')
    ...
    def track(self): #no toco este trackeo
        ...
        """
        if DEBUG1:
            print("Estoy en track del backend") 
        try:
            self.gaussian_fit()
    ...
    def correct(self, mode='continous'):
        ...
        if DEBUG1:
            print("Estoy en correct del backend") 
        dx = 0
        dy = 0
...
#                            currentZposition, pixeltime=10)
    ...
    def single_xy_correction(self, feedback_val, initial): 
        ...
        """
        if DEBUG1:
            print("Estoy en single_xy_correction del backend")   
        if DEBUG:
    ...
    def set_actuator_param(self, pixeltime=1000):
        if DEBUG1:
            print("Estoy en set_Actuator_param del backend")   
...
#        print(datetime.now(), '[xy_tracking] actuator x, y =', x_f, y_f)
        ...
        if DEBUG1:
            print("Estoy en actuator_xy del backend")   
    ...
    def set_moveTo_param(self, x_f, y_f, z_f, n_pixels_x=128, n_pixels_y=128,
        ...
        if DEBUG1:
            print("Estoy en set_moveTo_param del backend")   
    ...
    def moveTo(self, x_f, y_f, z_f): 
        ...
        if DEBUG1:
            print("Estoy en moveTo del backend")   
    ...
    def reset(self):
        ...
        if DEBUG1:
            print("Estoy en reset del backend")   
        self.initial = True
    ...
    def reset_data_arrays(self):
        if DEBUG1:
            print("Estoy en reset_data_arrays del backend")   
...
#        filename = fname + '_xydata.txt'
        if DEBUG1:
            print("Estoy en export_data del backend")   
        fname = self.filename
    ...
    def get_stop_signal(self, stoplive):
        ...
        """
        if DEBUG1:
            print("Estoy en get_stop_signal del backend")          
        self.toggle_feedback(False)
    ...
    def get_save_data_state(self, val):
        ...
        '''
        if DEBUG1:
            print("Estoy en get_save_data_state del backend")   
    ...
    def get_roi_info(self, N, coordinates_array): #Toma la informacion del ROI que viene de emit_roi_info en el frontend
        ...
        '''
        if DEBUG1:
            print("Estoy en get_roi_info del backend")   
    ...
    def get_lock_signal(self):
        ...
        '''
        if DEBUG1:
            print("Estoy en get_lock_signal del backend")   
        if not self.camON:
    ...
    def get_move_signal(self, r, r_rel): 
        ...
        if DEBUG1:
            print("Estoy en get_move_signal del backend")              
...
#        self.toggle_feedback(True) # TO DO: fix each position lock
    ...
    def start_tracking_pattern(self):
        if DEBUG1:
            print("Estoy en start_tracking_pattern del backend")   
        self.pattern = True
    ...
    def make_tracking_pattern(self, step):
        ...
        if DEBUG1:
            print("Estoy en make_tracking_pattern del backend")        
        if (step < 2) or (step > 5):
            return
        elif step == 2:
    ...
    def get_end_measurement_signal(self, fname):
        ...
        '''
        if DEBUG1:
            print("Estoy en get_end_mesurement_signal del backend")
        self.filename = fname
        self.export_data()
    ...
    def make_connection(self, frontend):
        if DEBUG1:
            print("Estoy en make_connection del backend")
        if DEBUG:
            print("Connecting backend to frontend")
            
        frontend.roiInfoSignal.connect(self.get_roi_info)
        print("roiInfoSignal from frontend connected to get_roi_info function in backend (slot)")
        frontend.closeSignal.connect(self.stop)
        print("closeSignal from frontend connected to stop function in backend (slot)")
        frontend.saveDataSignal.connect(self.get_save_data_state)
        print("saveDataSignal from frontend connected to get_save_data_state function in backend (slot)")
        ...
        print("tracking beads box conectado a toogle tracking conectado a tracking beads box..")
        ...
        print("liveviewButton from frontend connected to liveview in backend")
        ...
        print("feedbackLoopBox conectado a toogle feedback conectado a feedbackLoopBox..")
