...
class Frontend(QtGui.QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        fileMenu = menubar.addMenu('Mini microscope')
        ...
        ## xy tracking dock
        self.xyWidget = xy_tracking_flor.Frontend()
...
class Backend(QtCore.QObject):
    ...
    def __init__(self, adw, camera, *args, **kwargs):
        
        super().__init__(*args, **kwargs)
        
        self.zWorker = focus.Backend(camera, adw)
        self.xyWorker = xy_tracking_flor.Backend(camera, adw)
...
#    xyGUIThread.start()
    ...
    worker.xyWorker.adw.Set_FPar(70, pos_zero)
    worker.xyWorker.adw.Set_FPar(71, pos_zero)
    worker.xyWorker.adw.Set_FPar(72, pos_zero)
    
    worker.xyWorker.moveTo(10, 10, 10) # in µm
    
    time.sleep(0.200)
    gui.setWindowTitle('Mini_microscope')
    gui.showMaximized()
    app.exec_()
