...
#            self.roiInfoSignal.emit(roinumber, roicoordinates)
    ...
    def toggle_liveview(self, on):
        ...
        if on:
            ...
            print(datetime.now(), '[xy_tracking] Live view started- line 140')
        else:
            self.liveviewButton.setChecked(False)
            self.emit_roi_info()
            self.img.setImage(np.zeros((512, 512)), autoLevels=False)
            print(datetime.now(), '[xy_tracking] Live view stopped- line 145')
...
#        self.xyPoint.xyPointPlot.showGrid(x=True, y=True)
        ...
        print("liveviewButton connected to toggle liveview - line 427")
...
class Backend(QtCore.QObject):
    ...
    changedImage_tofocus = pyqtSignal(np.ndarray)
    ...
    def update_view(self):
        ...
        self.changedImage_tofocus.emit(self.image)
...
#        plt.imshow(array_sub, cmap=cmaps.parula, interpolation='None')
        ...
        bkg = np.min(array)
        A = np.max(array) - bkg
        σ = 130 # nm #en xyz_tracking el valor es 200
...
#                            currentZposition, pixeltime=10)
    ...
    def single_xy_correction(self, feedback_val, initial): 
        ...
        raw_image = self.camera.latest_frame()
        #self.image = raw_image[:, :, 0]
        self.image = np.sum(raw_image, axis=2)
        self.changedImage.emit(self.image)
...
#        self.toggle_feedback(True) # TO DO: fix each position lock
    ...
    def make_connection(self, frontend):
        ...
        print("liveviewButton connected liveview - line 333")
...
if __name__ == '__main__':
    ...
    print("Objeto gui frontend")
    gui = Frontend()
    print("Objeto worker backend")
