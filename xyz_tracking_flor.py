...
PX_SIZE = 29 # px size of camera in nm //Thorcam px size 29nm, IDScam 32nm
...
class Frontend(QtGui.QFrame):
    ...
    def toggle_liveview(self, on):
        ...
        if on:
            ...
            print(self.liveviewButton.setChecked(True))
        else:
...
#        self.xyzGraph.resize(200, 300)
        ...
        self.xyzGraph.xPlot = self.xyzGraph.addPlot(row=1, col=0) #modifiqué era row=0
        self.xyzGraph.xPlot.setLabels(bottom=('Time', 's'),
                                      left=('Y position', 'nm')) #decía X position
        self.xyzGraph.xPlot.showGrid(x=True, y=True)
                        
        self.xmeanCurve = self.xyzGraph.xPlot.plot(pen='b', width=40)
            
        self.xyzGraph.yPlot = self.xyzGraph.addPlot(row=0, col=0) #modifiqué era row=1
        self.xyzGraph.yPlot.setLabels(bottom=('Time', 's'),
                                     left=('X position', 'nm')) #decía X position
...
#        self.xyPoint.xyPointPlot.showGrid(x=True, y=True)
        ...
        self.zHist = pg.BarGraphItem(x=x, height=y, width=0.5, brush='#3BC14A')
        ...
        print(self.liveviewButton.isChecked())
...
class Backend(QtCore.QObject):
    ...
    def __init__(self, thorcam, piezo, *args, **kwargs):
        ...
        self.camera.auto_blacklevel = True #Se comenta para IDS
        self.camera.gain_boost = True #Se comenta para ids
    ...
    def liveview(self, value):
        print("Value con que se entra al liveview",value)
        if value:
            self.camON = True
            print("Entro a liveview_start desde liveview")
            self.liveview_start()
    ...
    def liveview_start(self):
        ...
        if self.camON:
            print("Estoy en el interior de liveview_start, valor de camON linea 674", self.camON)
...
#        print(datetime.now(), '[xy_tracking] entered update')
    ...
    def update_view(self):
        ...
        # self.image = np.sum(raw_image, axis=2)   # sum the R, G, B images #YA ESTABA COMENTADO
        self.image = raw_image[:, :, 0] # take only R channel #COMENTADO POR FLOR C
        #self.image = raw_image # ids Camera
...
#        plt.imshow(array_sub, cmap=cmaps.parula, interpolation='None')
    ...
    def make_connection(self, frontend):
        ...
        print("se ejecuta lineas make connection")
